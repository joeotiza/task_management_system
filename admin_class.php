...
Class Action {
	...
	function login(){
		extract($_POST);
			$qry = $this->db->query("SELECT * FROM referrer where reg_id = '".$reg_id."' and password = '".md5($password)."'  ");
		if($qry->num_rows > 0){
	...
	function save_referrer(){
		...
		$check = $this->db->query("SELECT * FROM referrer where reg_id ='$reg_id' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO referrer set $data");
		}else{
			$save = $this->db->query("UPDATE referrer set $data where id = $id");
		}
	...
	function delete_referrer(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM referrer where id = ".$id);
		if($delete)
			return 1;
	}
